...
import { CampaignProvider, CampaignProviderConfig, CampaignProviderForm } from '../../../types/interfaces/campaign-provider.interface';
...
export class CampaignProviderFormComponent implements OnInit {
  @Input() config!: CampaignProviderConfig;
  @Input() isSubmitting = false;
  @Input() initialData?: CampaignProvider | null = null;
  @Output() onSubmit = new EventEmitter<CampaignProviderForm>();
  ...
  ngOnInit() {
    ...
    this.config.fields.forEach(field => {
      if (this.initialData && field.name in this.initialData) {
        this.formData[field.name] = this.initialData[field.name as keyof CampaignProvider];
  ...
  submit() {
    ...
    const provider: CampaignProviderForm = {
      provider: this.config.value as CampaignProviderForm['provider'],
